...
export default function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-sky-950 backdrop-blur-md border-b border-slate-200 shadow-sm font-bold">
      <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
        ...
        >
          <Image src="/logo.png" alt="سين" width={50} height={100} />
        </Link>

        {/* Desktop Nav */}
        <div className="hidden md:flex items-center gap-6 text-sm font-medium text-slate-600">
          <Link href="/Posts" className="hover:text-sky-600 transition-colors">
            المقالات
          </Link>
          ...
          >
            <div className="px-4 py-4 flex flex-col gap-4 text-sm font-medium text-slate-600">
              ...
              {session ? (
                <>
                  {roles.some((r) =>
                    ["writer", "editor", "admin"].includes(r),
                  ) && (
                    <Link
                      href="/WriterPage"
                  ...
                  <button
                    onClick={() => signOut({ callbackUrl: "/" })}
                    className="flex items-center gap-2 text-red-500 text-right"
                  >
