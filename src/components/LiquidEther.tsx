...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class MouseClass {
      ...
      // Typed wrappers for global listeners (to avoid using 'any' and allow proper removal)
      ...
      init(container: HTMLElement) {
        ...
        // Listen globally so effect follows mouse even if other elements overlay the container
        window.addEventListener('mousemove', this._winMouseMove);
        window.addEventListener('touchstart', this._winTouchStart, { passive: true });
        window.addEventListener('touchmove', this._winTouchMove, { passive: true });
        window.addEventListener('touchend', this._winTouchEnd);
      }
      dispose() {
        ...
        window.removeEventListener('touchstart', this._winTouchStart);
        window.removeEventListener('touchmove', this._winTouchMove);
...
`;
    ...
    class ShaderPass {
      ...
      update(...args: unknown[]) {
        ...
        const target = this.props.output ?? null;
    ...
    interface AdvectionProps {
      ...
      dt: number;
      src: THREE.WebGLRenderTarget | null | undefined;
      dst: THREE.WebGLRenderTarget | null | undefined;
    }
    interface AdvectionUpdate { dt?: number; isBounce?: boolean; BFECC?: boolean }
    class Advection extends ShaderPass {
      line!: THREE.LineSegments;
      constructor(simProps: AdvectionProps) {
        const src = simProps.src!;
        const dst = simProps.dst!;
        super({
          material: {
    ...
    interface ExternalForceProps { cellScale: THREE.Vector2; cursor_size: number; dst: THREE.WebGLRenderTarget }
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(props?: ExternalForceUpdate) {
        const p = props || { cellScale: { x: 1, y: 1 }, cursor_size: 0 } as ExternalForceUpdate;
        const forceX = (Mouse.diff.x / 2) * (p.mouse_force || 0);
        const forceY = (Mouse.diff.y / 2) * (p.mouse_force || 0);
        const cellScale = p.cellScale || { x: 1, y: 1 };
        ...
        if (this.mouse && (this.mouse.material as THREE.RawShaderMaterial).uniforms) {
    ...
    interface ViscousUpdate { viscous?: number; iterations?: number; dt?: number }
    class Viscous extends ShaderPass {
      ...
      update(params: ViscousUpdate = {}): THREE.WebGLRenderTarget {
        const { viscous, iterations, dt } = params;
        if (!this.uniforms) {
          const fb = (this.props.output1 ?? this.props.output0) as THREE.WebGLRenderTarget;
          return fb;
        }
        // Resolve possibly-undefined props to nullable locals
        const out0: THREE.WebGLRenderTarget | null = this.props.output0 ?? null;
        const out1: THREE.WebGLRenderTarget | null = this.props.output1 ?? null;
        ...
        const fallback: THREE.WebGLRenderTarget | null = fbo_out ?? out1 ?? out0;
        // At construction time, dst/dst_ are provided, so fallback should exist
        return (fallback as THREE.WebGLRenderTarget);
      }
    }
    ...
    interface DivergenceUpdate { vel?: THREE.WebGLRenderTarget }
    class Divergence extends ShaderPass {
      ...
      update(params: DivergenceUpdate = {}) {
        const { vel } = params;
    ...
    interface PoissonUpdate { iterations?: number }
    class Poisson extends ShaderPass {
      ...
      update(params: PoissonUpdate = {}): THREE.WebGLRenderTarget {
        ...
        const fallback: THREE.WebGLRenderTarget | null = p_out ?? out1 ?? out0;
        return (fallback as THREE.WebGLRenderTarget);
      }
    }
    ...
    interface PressureUpdate { vel?: THREE.WebGLRenderTarget; pressure?: THREE.WebGLRenderTarget }
    ...
    class Simulation {
      ...
      resize() {
        this.calcSize();
        const list = Object.values(this.fbos);
        for (const rt of list) rt.setSize(this.fboSize.x, this.fboSize.y);
      }
      update() {
